...
median = cv2.medianBlur(img, 3)  # 블러링
gray = cv2.cvtColor(median, cv2.COLOR_BGR2GRAY)

blockSize = 4  # 이웃 화소 범위
apertureSize = 3  # 소벨 마스크 크기
k = 0.04
C = cv2.cornerHarris(gray, blockSize, apertureSize, k)  # OpenCV 제공 함수, 응답함수 계산 -> 코너일 확률

# 1 C값에 대한 thresholding
img[C > 0.05 * C.max()] = [0, 0, 255]   # 응답함수 최대값의 5%보다 큰 값은 코너로 검출 (임계값)
