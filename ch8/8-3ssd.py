...
# Detect objects
blob = cv2.dnn.blobFromImage(img, 1 / 127.5, (300 * width // height, 300), (127.5, 127.5, 127.5),
                             False)  # swapRB=True) # net에 입력되는 이미지를 blob 형식으로 변환
net.setInput(blob)  # blob 형태의 데이터를 net에 넣어줌
output = net.forward()  # net을 순방향으로 실행

for i in range(output.shape[2]):
    # print(output[0, 0, i])  # _(의미 없는 값), class_id(인식한 객체 번호), confidence(인식률), x, y, x2, y2(가로, 세로를 0~1로 정규화 하여 실수값 출력)
