...
mask[:, :] = cv2.GC_PR_BGD  # cv2.GC_INIT_WITH_MASK인 경우 반드시 cv2.GC_PR_BGD로 초기화
...
def painting(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        cv2.circle(mask, (x, y), BrushSiz, cv2.GC_FGD, -1)  # 물체로 설정
    elif event == cv2.EVENT_RBUTTONDOWN:
        cv2.circle(img_show, (x, y), BrushSiz, RColor, -1)  # 오른쪽 버튼 클릭하면 빨간색
        cv2.circle(mask, (x, y), BrushSiz, cv2.GC_BGD, -1)  # 배경으로 설정
