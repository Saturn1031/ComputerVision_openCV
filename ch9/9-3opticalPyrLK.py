...
cap = cv2.VideoCapture('checkBoard3x3.avi') # 'slow_traffic_small.mp4')
if not cap.isOpened():
    sys.exit('카메라 연결 실패')
...
p0 = cv2.goodFeaturesToTrack(prev_gray, mask=None, **feature_params)    # 특징점에 대한 정보 p0에 저장
...
while (1):
    ...
    for i in range(len(good_curr)):  # 이동 궤적 그리기
        ...
        mask = cv2.line(mask, (a, b), (c, d), color[i].tolist(), 2)    # line을 추가
